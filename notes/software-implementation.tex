...
\begin{itemize}
    ...
    \end{itemize}
    \item \textbf{Spectrogram Parameters:} Settings for scipy's spectrogram function (\verb|spec_nperseg|, \verb|spec_noverlap|, 
    and, \verb|spec_window|)
    ...
    \end{itemize}
    \item \textbf{Short-Time Fourier Transform (STFT) Parameters:} Settings for 2D time-frequency representation.
    \begin{itemize}
...
\subsubsection{General Processing Framework}
...
\begin{enumerate}
    \item \textbf{Step 1: Load Raw Data} \\ 
    \begin{verbatim}
        load_raw_data(filepath) -> (P_all, frequency, signal_all, 
                                    amplitude_all, m_dot_all)
    \end{verbatim} 
    ...
    \item \textbf{Step 2: Compute Power Spectral Density using Welch's Method} \\
    \begin{verbatim}
    ...
    \item \textbf{Step 3: Frequency Range Filtering} \\
    ...
    \item \textbf{Step 4: Generate STFT Spectrograms for 2D Representation} \\
    \begin{verbatim}
        generate_stft_spectrograms(signal_matrix, fs) -> (frequencies, times, 
                                                            stft_normalized)
    \end{verbatim}
    ...
    \item \textbf{Step 5: Frequency Range Filtering} \\
...
\begin{enumerate}
    \item \textbf{WhistleDataset Class (1D PSD)} \\
    ...
    \item \textbf{WhistleDataset2D Class (2D STFT)} \\
    ...
    \item \textbf{get\_dataloaders() Function (1D PSD)} \\
    \begin{verbatim}
    ...
    \item \textbf{get\_dataloaders\_2d() Function (2D STFT)} \\
    \begin{verbatim}
        train_loader_2d, test_loader_2d = get_dataloaders_2d(batch_size=16, 
                                                            test_size=0.2)
    \end{verbatim}
...
\begin{enumerate}
    \item \textbf{Convolutional Feature Extractor}
    ...
    \item \textbf{Fully Connected Regressor:}
    \begin{verbatim}
...
A 2D convolutional architecture designed for time-frequency spectrograms:
...
\begin{enumerate}
    ...
    \end{verbatim}
    Generates plots from original visualization pipeline.
\end{enumerate}
